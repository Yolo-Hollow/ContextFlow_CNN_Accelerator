...
> **路径替换说明：** 命令中的 `C:\Xilinx\...`、`E:\COCO80_R5` 以及所有 `<...>` 内容均为示例或占位符。请按本机 Vitis 安装位置、SD 卡盘符、工作区和模型产物的实际绝对路径替换，并去掉尖括号。相对路径需要从仓库根目录执行。
...
部署工具不会格式化 SD 卡，也不会覆盖内容不同的同名文件。以下以 `E:` 为例，使用前必须替换为实际 SD 卡盘符：
