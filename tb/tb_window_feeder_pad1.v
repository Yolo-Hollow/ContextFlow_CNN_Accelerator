...
module tb_window_feeder_pad1;
    ...
    integer push_count, exp_oy, exp_ox, fy, fx, exp_val;
    integer fill_count;
    ...
    initial begin
        ...
        for (b = 0; b < 5; b = b + 1) begin
            dma_wr_data[b] = 0;
            for (fy = 0; fy < FM_H; fy = fy + 1)
                for (x = 0; x < FM_W; x = x + 1)
                    feat[b][fy][x] = b*40 + fy*8 + x + 1;
        end
    ...
    initial begin
        ...
        forever begin
            wait(fill_req);
            if (fill_fy !== fill_count[AW-1:0]) begin
                $display("[FAIL] fill%0d got fy=%0d exp=%0d", fill_count, fill_fy, fill_count);
                fail = fail + 1;
            end else begin
                pass = pass + 1;
            end
    ...
    always @(posedge clk) begin
        if (ifm_valid) begin
            exp_oy = push_count / FM_W;
            exp_ox = push_count % FM_W;
            ...
            for (lane = 0; lane < 32; lane = lane + 1) begin
                ...
                if (ch < 5) begin
                    ...
                    if (fy < 0 || fy >= FM_H || fx < 0 || fx >= FM_W)
                        ...
                        exp_val = feat[ch][fy][fx];

                    if (ifm_data[lane*8 +: 8] !== exp_val[7:0]) begin
                        $display("[FAIL] push%0d lane%0d got=%0d exp=%0d fy=%0d fx=%0d",
                            push_count, lane, ifm_data[lane*8 +: 8], exp_val, fy, fx);
                        fail = fail + 1;
                    end else begin
                        pass = pass + 1;
                    end
    ...
    initial begin
        wait(done);
        @(negedge clk);
        if (push_count != FM_H*FM_W) begin
            $display("[FAIL] push count got=%0d exp=%0d", push_count, FM_H*FM_W);
            fail = fail + 1;
        end else begin
            pass = pass + 1;
        end
        if (fill_count != FM_H) begin
            $display("[FAIL] fill count got=%0d exp=%0d", fill_count, FM_H);
            fail = fail + 1;
        end else begin
            pass = pass + 1;
        end
        $display("=== tb_window_feeder_pad1: %0d pass, %0d fail ===", pass, fail);
        if (fail != 0) $fatal(1);
        $finish;
    end
    ...
    initial begin
        ...
        $display("[FAIL] timeout push_count=%0d fill_count=%0d busy=%0d done=%0d oy=%0d ox=%0d ready=%0d",
            push_count, fill_count, busy, done, cur_oy, cur_ox, window_ready);
        $fatal(1);
    end
endmodule
