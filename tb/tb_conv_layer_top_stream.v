...
module tb_conv_layer_top_stream;
    ...
    reg [COLS*2*PSUM_W-1:0] final_pkt [0:COUT_TOTAL/COUT_TILE-1][0:PIXELS-1];
        ...
        begin
            ...
            for (kk = 0; kk < ROWS; kk = kk + 1) begin
                for (cc = 0; cc < COUT_TILE; cc = cc + 1) begin
                    ...
                    wgt_tile_wr_data = (gk < K_TOTAL) ? weight[gk][co_base + cc] : 8'd0;
                end
            end
            @(negedge clk);
            wgt_tile_wr_en = 1'b0;
    ...
    initial begin
        ...
        if (final_count != PIXELS * (COUT_TOTAL / COUT_TILE)) begin
