...
module tb_layer_scheduler_stream;
    localparam K_TILE = 32;
    localparam COUT_TILE = 8;
    localparam K_TOTAL = 45;
    localparam COUT_TOTAL = 16;
    localparam NUM_PIXELS = 9;
    ...
    integer exp_cout [0:3];
    integer exp_k [0:3];
    integer exp_first [0:3];
    integer exp_final [0:3];
    ...
    initial begin
        forever begin
            ...
            if (bias_load_start) begin
                check_equal(cout_base, (bias_count == 0) ? 0 : 8, "bias cout");
                bias_count = bias_count + 1;
                bias_delay = 1;
            end
            if (weight_load_start) begin
                check_equal(cout_base, exp_cout[weight_count], "weight cout");
            ...
            if (feeder_start) begin
                check_equal(cout_base, exp_cout[feeder_count], "feeder cout");
                check_equal(pass_base_k, exp_k[feeder_count], "feeder k");
                feeder_count = feeder_count + 1;
                feeder_delay = 2;
            end
            if (compute_start) begin
                check_equal(cout_base, exp_cout[compute_count], "compute cout");
            ...
            if (psum_drain_start) begin
                check_equal(cout_base, exp_cout[drain_count], "drain cout");
    ...
    initial begin
        ...
        exp_cout[0] = 0; exp_k[0] = 0;  exp_first[0] = 1; exp_final[0] = 0;
        exp_cout[1] = 0; exp_k[1] = 32; exp_first[1] = 0; exp_final[1] = 1;
        exp_cout[2] = 8; exp_k[2] = 0;  exp_first[2] = 1; exp_final[2] = 0;
        exp_cout[3] = 8; exp_k[3] = 32; exp_first[3] = 0; exp_final[3] = 1;
        ...
        check_equal(bias_count, 2, "bias count");
        check_equal(weight_count, 4, "weight count");
        check_equal(feeder_count, 4, "feeder count");
        check_equal(compute_count, 4, "compute count");
        check_equal(drain_count, 4, "drain count");
        check_equal(busy, 0, "busy clear");

        $display("=== tb_layer_scheduler_stream: %0d pass, %0d fail ===", pass, fail);
        if (fail != 0) $fatal(1);
        $finish;
    end
    ...
    initial begin
        ...
        $display("[FAIL] timeout bias=%0d weight=%0d feeder=%0d compute=%0d drain=%0d",
            bias_count, weight_count, feeder_count, compute_count, drain_count);
        $fatal(1);
    end
endmodule
