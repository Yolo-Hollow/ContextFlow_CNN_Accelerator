...
- Board validation is pending. Current host enumeration only shows Bluetooth COM3/COM4 and `xsct probe_jtag_targets.tcl` prints no JTAG targets, so `Cortex-A53 #0` and the KV260 UART are not usable in this session. After reconnecting or power-cycling the board until COM8 and JTAG reappear, run full programming with `build_system_xck26_kv260_drainpipe`, then run the two DDR demos and compare `STAGEPERF drain` against the `stageperf` baseline `30102432` cycles.
