...
- Not yet done: no Vivado implementation or board validation for raw HWC cache. The first board target should be a directed Conv9/Conv7 native `1x1` smoke on a new `build_system_xck26_kv260_hwccache_1x1_2022_2` build. Do not enable raw HWC for `3x3`, pooling layers, or default Conv0->Conv9 DDR demo until that directed board test passes.
