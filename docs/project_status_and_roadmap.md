...
- Board validation on `COM8` with full PL programming found that `RawHwcComputeStartLevel=64` is not safe: Conv0-4 complete, then Conv5 raw-HWC tile0 times out with `CTRL=0x00000001`. Rebuilding only the ELF with `RawHwcComputeStartLevel=0` on the same bitstream passes the fixed-image DDR demo with unchanged `with_mask` detection and total `PERF total_us=544.490 ms`, matching the serialized raw-HWC baseline.
- A second nonzero test with `RawHwcComputeStartLevel=1024` also times out at Conv5 tile0. Debug reads while stuck show `TAIL_CONFIG=0x04000001`, `VECTOR_PACKETS=1`, `VECTOR_PIXELS=52`, `COMP_FIRE=52`, `STAGE_DRAIN=0`, `RAW_REPLAY_ACTIVE=104`, and `RAW_LOAD_ACTIVE` still accumulating. This means compute was allowed to run one pixel pass while the raw-HWC cache/load/replay transaction was not complete, then the scheduler remained in compute wait because feeder-done never arrived.
- Current conclusion: the overlap mechanism must remain disabled by default. The next design fix should use an explicit raw-cache transaction boundary or producer/consumer credit tracking, not a simple total-push watermark. A safe design needs to start compute only when replay can continue independently through all required K/COUT packets, and drain must not depend on a feeder-done event that can be blocked by a concurrent cache load/replay state.
