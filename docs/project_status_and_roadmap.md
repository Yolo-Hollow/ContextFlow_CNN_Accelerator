...
## 15. 2026-06-09 3x3 raw-HWC cache directed prototype

- Extended the experimental `axis_hwc_tile_cache` beyond native `1x1` to a directed `3x3` mode. The existing default remains `raw_hwc_mode=0`, so the board-validated prepacked IFM path is unchanged unless software explicitly sets `STREAM_CFG[1]`.
- The `3x3` prototype keeps the existing vector feeder interface and replays 18 IFM lanes per K pass. To make this possible without a 9-way read crossbar, the cache stores raw HWC input into a replicated global-K layout: `global_k = channel * 9 + kernel_pos`, `bank = global_k % ROWS`, and `addr = pixel * ceil(K_TOTAL/ROWS) + global_k/ROWS`. Padding/out-of-range window positions replay internal signed zero.
- This layout is intentionally a performance/architecture probe, not a final resource-optimized design. For a Conv6 `13x4` tile with `CIN=512`, it uses `HWC_CACHE_AW=15` in simulation because the 3x3 replicated layout is much larger than the earlier native `1x1` AW12 cache. Synthesis/resource evaluation is required before considering a board build or expanding this to the default scheduler.
- Added `tb_conv_accel_core_axi_lite_axis_stream_conv6_3x3_raw_hwc_ext_tile0_cout16` as a targeted external-golden xsim test for Conv6 tile0, first COUT tile only. It uses `ROWS=18`, `COLS=8`, `COUT_TILE=16`, `CIN=512`, `OFM=13x13`, `tile_ofm_h=4`, `input_zp=19`, and external RTL-semantic memories under `D:/MPSoC/python_prj/rtl_golden/facemask_chain_conv0_conv6_rtl/06_head_conv6_3x3/xsim_mem_cout16`.
- Vivado/xsim `2022.2` validation passed: `tb_axis_hwc_tile_cache` is now `259/0`, the new Conv6 `3x3` raw-HWC directed test is `854/0`, existing Conv7 native `1x1` raw-HWC tile0 remains `13334/0`, and legacy prepacked r18_c8 Layer06 tile4 remains `26641/0`.
- Current conclusion: the prototype proves that a raw-HWC cache can feed the existing compute path for a real `3x3` Conv6 tile bit-exactly. The next decision should be based on synthesis cost and expected feeder-cycle reduction; a production version may need a less replicated bank layout, deeper scheduling changes, or a cache/replay design that trades one-cycle replay for lower BRAM use.
