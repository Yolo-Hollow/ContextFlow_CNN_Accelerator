...
YOLOv3-tiny 网络中主要使用 3×3 卷积（$K_h=K_w=3$）和 1×1 卷积（通道变换层）。本文加速器专门针对 3×3 卷积的硬件友好展开进行了优化；从数学形式上看，1×1 卷积可通过将 3×3 卷积核中非中心位置权重置零来映射为特例，但当前 RTL 实现与验证主要围绕 3×3 窗口流展开，1×1 的专用调度优化属于后续可扩展方向。
