...
//   AXI byte address [9:2] -> cfg_addr[7:0]
//
...
module axi_lite_cfg_bridge #(
    ...
    output reg [ADDR_W-3:0] cfg_addr,
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (do_write) begin
                cfg_addr <= write_addr[ADDR_W-1:2];
                aw_hold_valid <= 1'b0;
                w_hold_valid <= 1'b0;
                if (write_strb == {DATA_W/8{1'b1}} ||
                    write_addr[ADDR_W-1:2] == {(ADDR_W-2){1'b0}}) begin
                    cfg_wdata <=
                        (write_addr[ADDR_W-1:2] == {(ADDR_W-2){1'b0}}) ?
                        apply_wstrb_mask(write_data, write_strb) : write_data;
                    cfg_wr_en <= 1'b1;
                    s_axi_bresp <= 2'b00;
                    s_axi_bvalid <= 1'b1;
                end else begin
            ...
            if (wr_state == WR_MERGE) begin
                cfg_addr <= wr_merge_addr[ADDR_W-1:2];
            ...
            case (rd_state)
                RD_IDLE: begin
                    if (s_axi_arvalid && s_axi_arready) begin
                        cfg_addr <= s_axi_araddr[ADDR_W-1:2];
                        cfg_rd_en <= 1'b1;
                        rd_state <= RD_WAIT;
                    end
                end
